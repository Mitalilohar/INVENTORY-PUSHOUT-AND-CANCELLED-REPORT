CREATE OR REPLACE PROCEDURE APPS.xx_ascp_old_plan_prc2 (
   p_plan_id          IN   NUMBER,
...
IS
BEGIN
   DELETE FROM bolinf.xx_gtt_ascp_old_tbl;

   INSERT INTO bolinf.xx_gtt_ascp_old_tbl
               (org_id, component, allocated_qty, inventory_item_id,
                sr_inventory_item_id, CATEGORY, attribute2)
      SELECT   organization_id, component,
               SUM (allocated_quantity) AS allocated_qty, inventory_item_id,
               sr_inventory_item_id,
               REGEXP_REPLACE (category_name_end_item, '^PRODUCT\.', ''),
               MAX(
  NVL(
...
EXCEPTION
   ...
   THEN
      fnd_file.put_line (fnd_file.LOG,
                         'Error in xx_ascp_old_plan_prc: ' || SQLERRM
                        );
END;
/
