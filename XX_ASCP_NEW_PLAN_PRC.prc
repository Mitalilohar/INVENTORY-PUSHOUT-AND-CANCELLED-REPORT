CREATE OR REPLACE PROCEDURE APPS.xx_ascp_new_plan_prc (
   p_plan_id   IN   NUMBER,
...
IS
BEGIN
   DELETE FROM bolinf.xx_gtt_ascp_new_tbl;

   INSERT INTO bolinf.xx_gtt_ascp_new_tbl
               (org_id, component, allocated_qty, inventory_item_id,
                sr_inventory_item_id)
      SELECT   organization_id, component,
               SUM (allocated_quantity) AS allocated_qty, inventory_item_id,
               sr_inventory_item_id
          FROM (SELECT   (SELECT organization_code
                         ...
                         msi2.item_name child_component, msi1.inventory_item_id,
                   ...
                   WHERE msd1.plan_id = mfp1.plan_id
                     ...
                     --AND trunc(mss2.new_wip_start_date) = trunc(to_date('03/04/2025','DD/MM/YYYY'))
...
EXCEPTION
   ...
   THEN
      fnd_file.put_line (fnd_file.LOG,
                         'Error in xx_ascp_new_plan_prc: ' || SQLERRM
                        );
END;
/
...
BEGIN
   xx_ascp_new_plan_prc (8022, TO_DATE ('2025-08-31', 'YYYY-MM-DD'), 727);
END;
/

BEGIN
   xx_ascp_new_plan_prc (8022, 727,TO_DATE ('2025-07-07', 'YYYY-MM-DD'),TO_DATE ('2025-08-31', 'YYYY-MM-DD'));
END;
/

SELECT * FROM bolinf.xx_gtt_ascp_new_tbl